        ...
        if (differenceInMilliseconds <= 1800000 && differenceInMilliseconds > 0 && stringData[row]["F"] != true) {//1800000
          ...
          if (client.user) {
            ...
            if(user != undefined) {
              ...
              if (content[user.id].orarendColor != "") {
                embed.setColor(Colors.Aqua)
              }
              if ((line["Összefoglalás"] as string)[(line["Összefoglalás"] as string).indexOf(")") - 1].toLowerCase() != "e" && content[user.id].wantEloadas == false) {
                user.send({ embeds: [embed] }).then(() => { }).catch((error) => {
                  Logger.log(`${error}\nCan't send message to user`, "error")
                })
              } else if(content[user.id].wantEloadas == true) {
                user.send({ embeds: [embed] }).then(() => { }).catch((error) => {
                  Logger.log(`${error}\nCan't send message to user`, "error")
                })
              }
